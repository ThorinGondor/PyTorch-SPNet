...
class YOLOLoss(nn.Module):
    def __init__(self, anchors, num_classes=80, img_size=(416, 416)): # anchors[0][1][2], 80, (416, 416)
        super(YOLOLoss, self).__init__()
        self.anchors = anchors
        self.num_anchors = len(anchors) # 3
        self.num_classes = num_classes # 80
        self.bbox_attrs = 5 + num_classes # 5: 对角坐标xy+置信度，num_classes: 各类概率
        self.img_size = img_size

        self.ignore_threshold = 0.5
        self.lamda_xy = 2.5 # 坐标误差的惩罚系数
        self.lamda_wh = 2.5 # 坐标误差的惩罚系数
        self.lamda_conf = 1.0 # 置信度的惩罚系数
        self.lamda_cls = 1.0 # 类别概率的惩罚系数

        self.mse_loss = nn.MSELoss().cuda()
        self.bce_loss = nn.BCELoss().cuda()

    def forward(self, input, targets=None): # loss的输入即网络的输出结果
        bs = input.size(0)  # 网络的？
        in_h = input.size(2) # 网络预测结果的高度
        in_w = input.size(3) # 网络预测结果的宽度
        ...
        # Get outputs：prediction里存储的是全部的预测结果
        x = torch.sigmoid(prediction[..., 0])         # Center x
        y = torch.sigmoid(prediction[..., 1])         # Center y
        w = prediction[..., 2]                        # Width
        h = prediction[..., 3]                        # Height
        conf = torch.sigmoid(prediction[..., 4])      # Conf
        pred_cls = torch.sigmoid(prediction[..., 5:]) # Classes pred
        ...
        if targets is not None:
            ...
            # 求算总体loss：total loss = losses * weight
            loss = loss_x * self.lamda_xy + loss_y * self.lamda_xy + \
                   loss_w * self.lamda_wh + loss_h * self.lamda_wh + \
                   loss_conf * self.lamda_conf + loss_cls * self.lamda_cls
