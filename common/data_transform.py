...
class Compose(object):
    """Composes several transforms together. 各种图像变换技术
    Args:
        transform (list of ``Transform`` objects): list of transforms to compose.
    """
...
class ToTensor(object): # 将图片转为Tensor张量
    def __init__(self, max_objects = 50, is_debug=False):
        self.max_objects = max_objects
        self.is_debug = is_debug

    def __call__(self, sample): # 输入参量：原始样本集(images, labels)
        image, labels = sample['image'], sample['label']
        # images转化为Tensor张量
        if self.is_debug == False:
            image = image.astype(np.float32)
            image /=  255.0
            # 转置数组 axis（0，1，2）———>axis(2，1，0) ，transpose后的数组相对于原数组来说，相当于交换了原数组的0轴和2轴
            image = np.transpose(image, (2, 0, 1))
            image = image.astype(np.float32)
        # labels转化为Tensor张量
        filled_labels = np.zeros((self.max_objects, 5), np.float32) # zeros(50x5)
